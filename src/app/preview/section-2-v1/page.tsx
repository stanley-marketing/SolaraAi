...
function TileCard({ capability }: { capability: Capability }) {
  const Icon = capability.icon;
  return (
    <div
      className="group relative flex aspect-square flex-col items-center justify-center gap-3 rounded-2xl border bg-white p-5 transition-all duration-200 ease-out hover:-translate-y-0.5"
      style={{
    ...
    <main className="min-h-screen bg-white">
      ...
      >
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-14 grid grid-cols-3 gap-2.5 sm:mt-20 sm:grid-cols-4 sm:gap-3 lg:grid-cols-6">
            {CAPABILITIES.map((cap) => (
              <TileCard key={cap.label} capability={cap} />
            ))}
          </div>
      ...
      <div className="mx-auto max-w-3xl px-6 py-12 text-center">
        ...
        >
          Variation 1 &middot; Bordered Tiles
        </p>
        <p className="mt-3 text-sm text-ink-700/70">
          Each capability sits in its own square tile: soft 7%-opacity border,
          rounded-2xl corners, subtle 1px shadow. Hover lifts the tile
          -0.5px. Premium, product-page feel. Clean Apple / Linear language.
        </p>
      </div>
    </main>
  );
